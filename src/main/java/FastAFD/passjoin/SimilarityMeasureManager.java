...
public class SimilarityMeasureManager {
    ...
    //compute different kind of similarity
    public SimilarityMeasureManager(int absoluteEditDistance, float similarityThreshold, boolean hybridMode, boolean tokenMode) {
        if (similarityThreshold == 0 && !hybridMode && !tokenMode) {
            ...
            if (tokenMode && !hybridMode) {
                ...
                throw new RuntimeException("False configuration of similarity threshold");
            }
        } else {
            throw new RuntimeException("False configuration of similarity measure");
        }
    }
